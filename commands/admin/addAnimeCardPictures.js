...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName("editimage")
        .setDescription("Add, update, or remove an image from the anime card pictures database")
        ...
        .addBooleanOption(option =>
            option
                .setName("remove")
                .setDescription("Set to true if you want to remove an existing image, false otherwise")
                .setRequired(false) // Make this option optional
        ),
    async execute(interaction) {
        ...
        try {
            console.log(`Processing command with options: cardId=${cardId}, imageId=${imageId}, imageUrl=${imageUrl}, update=${update}, remove=${remove}`);

            if (remove) {
                if (!imageId) {
                    await interaction.reply("Please provide the image ID to remove.");
                    return;
                }
                ...
                await dbRunAsync(`
                    ...
                    WHERE id = ?;
                `, [imageId]);
                await interaction.reply(`Image with ID ${imageId} removed successfully!`);
                return;
            }
            ...
            if (update) {
                if (!cardId) {
                    await interaction.reply("Please provide the card ID to update the image.");
                    return;
                }
            ...
            await interaction.reply("An error occurred while adding, updating, or removing the image.");
        }
    },
};
