...
async function createOrInsertUser(categoryName, categoryPower, categoryResistance, categoryVariability, critRate, critPower, domainLevel, update) {
    try {
        ...
        if (update) {
            ...
            WHERE id = ?;
            `, [categoryName, categoryPower, categoryResistance, categoryVariability, critRate, critPower, domainLevel, update]);
            console.log(`Card "${categoryName}" (ID ${update}) updated in the database.`);
        } else {
            ...
            await dbRunAsync(`
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?);
            `, [categoryName, categoryPower, categoryResistance, categoryVariability, critRate, critPower, domainLevel]);
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        ...
        .addIntegerOption(option =>
            option
                .setName("domainlevel")
                .setDescription("(3. Highest Domain (Light vs Dark) 2. Quirky Domain 3. Attribute Domain)")
                .setRequired(true)
        )
        .addIntegerOption(option =>
            option
                .setName("update")
                .setDescription("Set to true if you want to update an existing card, false otherwise")
        ),
    async execute(interaction) {
        ...
        const categoryResistance = interaction.options.getString("categoryresistance");
        const categoryVariability = interaction.options.getInteger("categoryvariability");
        const categoryCritRate = interaction.options.getInteger("categorycritrate");
        const categoryCritPower = interaction.options.getInteger("categorycritpower");
        ...
        await createOrInsertUser(categoryName, categoryPower, categoryResistance, categoryVariability, categoryCritRate, categoryCritPower, domainLevel, updateCardId);

        // Respond to the interaction
        await interaction.reply(updateCardId
